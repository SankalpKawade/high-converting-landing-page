...
// Reusable Feature Card maintaining exact dimensions and design
function FeatureCard({
  item,
  id,
}: {
  item: typeof mainSections[0];
  id?: string;
}) {
  return (
    <div
    ...
    >
      ...
      >
        ...
        {/* Mockup image/video at the bottom */}
...
export default function FeatureGrid() {
  ...
  // Duplicate items to allow for seamless infinite scrolling on desktop
  const extendedSections = [...mainSections, ...mainSections, ...mainSections, ...mainSections];

  useEffect(() => {
    // Only run infinite scroll animation on desktop browsers (>= 768px)
    ...
    const startAnimation = () => {
      ...
      const scrollLoop = () => {
        if (!isHovered && el && isDesktop) {
          el.scrollLeft += scrollSpeed;

          // Seamless reset when reaching midpoint
  ...
  return (
    <section className="py-12 sm:py-20 px-4 relative z-10" id="features">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {/* MOBILE SCREENS: Each box visible one after other without infinite scroll */}
        <div className="flex flex-col items-center gap-6 md:hidden w-full pb-6">
          {mainSections.map((item, i) => (
            <FeatureCard
              key={i}
              item={item}
              id={item.cardTitle === "Fast-Track Editing Course" ? "fast-track-course" : undefined}
            />
          ))}
        </div>

        {/* DESKTOP BROWSERS: Infinite scroll animation carousel */}
        <div
          onMouseEnter={() => setIsHovered(true)}
          onMouseLeave={() => setIsHovered(false)}
          className="relative group w-full pb-6 hidden md:block"
        >
          {/* Edge gradients for smooth fade-out */}
          <div className="absolute top-0 bottom-0 left-0 w-24 bg-gradient-to-r from-[#040810] to-transparent z-20 pointer-events-none"></div>
          <div className="absolute top-0 bottom-0 right-0 w-24 bg-gradient-to-l from-[#040810] to-transparent z-20 pointer-events-none"></div>
          ...
          >
            {extendedSections.map((item, i) => (
              <FeatureCard
                key={i}
                item={item}
                id={i === 5 ? "fast-track-course-desktop" : undefined}
              />
            ))}
          </div>
        </div>
