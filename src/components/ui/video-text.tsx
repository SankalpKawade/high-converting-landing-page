...
export function VideoText({
  ...
  useEffect(() => {
    const updateSvgMask = () => {
      ...
      const textInner = lines.map((line, i) => {
        const dy = i === 0 ? `-${(lines.length - 1) * 0.55}em` : `1.1em`;
        return `<tspan x='50%' dy='${dy}'>${line}</tspan>`;
      }).join('')

      const newSvgMask = `<svg xmlns='http://www.w3.org/2000/svg' width='100%' height='100%'><text x='50%' y='50%' font-size='${responsiveFontSize}' font-weight='${fontWeight}' text-anchor='${textAnchor}' dominant-baseline='${dominantBaseline}' font-family='${fontFamily}'>${textInner}</text></svg>`
